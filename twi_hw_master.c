 ...
 * the file.
 * 
 * Modified by Tom Magnier <magnier.tom@gmail.com> 16/07/14
 * to allow using a SoftDevice (with Nordic SDK v5.2 there was a problem when
 * using PPI causing Hard Faults)
 */
...
static bool twi_master_read(uint8_t *data, uint8_t data_length, bool issue_stop_condition)
{
    ...
    {
#if(TWI_USE_WITH_SOFTDEVICE == 0)
        NRF_PPI->CH[0].TEP = (uint32_t)&NRF_TWI1->TASKS_STOP;
#else
		uint32_t err_code = sd_ppi_channel_assign(0, &NRF_TWI1->EVENTS_BB, &NRF_TWI1->TASKS_STOP);
...
#else
		uint32_t err_code = sd_ppi_channel_assign(0, &NRF_TWI1->EVENTS_BB, &NRF_TWI1->TASKS_SUSPEND);
		APP_ERROR_CHECK(err_code);
#endif
    }
...
#else
	uint32_t err_code = sd_ppi_channel_enable_set(PPI_CHENSET_CH0_Msk);
	APP_ERROR_CHECK(err_code);
#endif
